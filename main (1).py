...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# FAISS DISK CACHE — reuse indexes across identical uploads
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# During a hackathon demo the same sample PDF often gets uploaded many times
# across different browser sessions. Without this, every one of those re-embeds
# the whole document and burns the shared free-tier Gemini embedding quota for
# nothing. Caching by content hash means an identical file is only ever
# embedded once per running instance.
...
def get_llm():
    """Create (and reuse) a ChatGoogleGenerativeAI instance with safe settings.

    Streamlit reruns this whole script on every widget interaction. Without
    caching, a brand-new API client was being constructed on every keystroke
    / button click, which was a real contributor to the app feeling laggy.
    """
...
@st.cache_resource(show_spinner=False)
def get_embeddings():
    """Create (and reuse) the embeddings client across reruns."""
    return GoogleGenerativeAIEmbeddings(model="models/gemini-embedding-001")


# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# HELPER — Resilient calls to the Gemini API
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
def is_quota_error(e: Exception) -> bool:
    """True if an exception is a 429 / RESOURCE_EXHAUSTED rate-limit error."""
    msg = str(e)
    return "RESOURCE_EXHAUSTED" in msg or "429" in msg or "quota" in msg.lower()


def extract_retry_delay(e: Exception, default: float) -> float:
    """Use the server's suggested retryDelay (e.g. '30s') from the error, if present."""
    match = re.search(r"retryDelay['\"]?\s*:\s*['\"]?(\d+(?:\.\d+)?)", str(e))
    return float(match.group(1)) if match else default


def call_with_backoff(fn, *args, max_retries: int = 4, base_delay: float = 5.0, on_retry=None, **kwargs):
    """Call fn(*args, **kwargs), retrying with exponential backoff on Gemini
    429 / RESOURCE_EXHAUSTED errors (the free-tier rate limit is shared across
    everyone using the app, so a transient hit is common and usually clears
    within seconds). Any other kind of error is raised immediately, since
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            if not is_quota_error(e) or attempt == max_retries:
                raise
            wait = extract_retry_delay(e, default=base_delay * (2 ** attempt))
            if on_retry:
        ...
        if st.session_state.processed_file_id != file_id:
            with st.spinner("🔄 Ingesting document..."):
                try:
                    ...
                    # 5. Create/reuse embeddings + vector store
                    embeddings = get_embeddings()

                    # Hash the file's bytes so identical uploads (very common
                    # during a hackathon demo, where lots of people test the
                    # same sample PDF) reuse an existing index on disk instead
                    # of burning embedding quota re-computing it.
                    ...
                    if os.path.isdir(cache_path):
                        ...
                        def _on_retry(attempt, max_attempts, wait_s):
                            retry_status.info(
                                f"⏳ Gemini's free-tier embedding limit was hit. "
                                f"Retrying in {wait_s:.0f}s (attempt {attempt}/{max_attempts})…"
                            )

                        vector_store = call_with_backoff(
                            FAISS.from_documents, chunks, embeddings, on_retry=_on_retry
                        )
                    ...
                    if is_quota_error(e):
                        st.error(
                            "⏳ **Gemini's free-tier embedding quota was hit**, even after "
                            "automatic retries. This quota is shared across everyone using "
                            "the app right now — please wait about a minute and try uploading again."
                        )
                    else:
    ...
    with st.chat_message("assistant"):
        with st.spinner("🔍 Searching knowledge base..."):
            try:
                ...
                if is_quota_error(e):
                    full_response = (
                        "⏳ **Gemini's free-tier rate limit was hit** and automatic retries "
                        "didn't clear it in time. This quota is shared across everyone using "
                        "the demo right now — please wait about a minute and ask again."
                    )
                else:
                    full_response = (
